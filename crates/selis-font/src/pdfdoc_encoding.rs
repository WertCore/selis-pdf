//! PDFDocEncoding (SL-3.TEXT.02) — ISO 32000-2:2020 Annex D.6.
//!
//! The PDF document encoding: bytes 0–127 are ASCII, 128–255 map to the
//! characters below. Undefined bytes 0xFE/0xFF map to U+FFFD.

/// PDFDocEncoding: byte → Unicode scalar value.
pub const PDF_DOC_ENCODING: [u32; 256] = [
    ...
    0x0078, 0x0079, 0x007A, 0x007B, 0x007C, 0x007D, 0x007E, 0x007F, 0x2022, 0x2020, 0x2021, 0x2026,
    ...
    0x0153, 0x0161, 0x017E, 0x00A1, 0x00A2, 0x00A3, 0x00A4, 0x00A5, 0x00A6, 0x00A7, 0x00A8, 0x00A9,
    ...
    0x00FE, 0x00FF, 0xFFFD, 0xFFFD,
];

/// Decode a PDFDocEncoding byte.
#[must_use]
...
mod tests {
    ...
    fn ascii_bytes_are_identity() {
        assert_eq!(decode(0x41), 0x41); // 'A'
        assert_eq!(decode(0x20), 0x20); // space
    }

    #[test]
    fn high_bytes_map_to_unicode() {
        assert_eq!(decode(0x80), 0x2022); // bullet
        assert_eq!(decode(0x83), 0x2026); // ellipsis
        assert_eq!(decode(0x8F), 0x2018); // quoteleft
        assert_eq!(decode(0xE4), 0x00E6); // ae
        assert_eq!(decode(0xFD), 0x00FF); // ydieresis
    }

    #[test]
    fn undefined_bytes_are_replacement() {
        assert_eq!(decode(0xFE), 0xFFFD);
        assert_eq!(decode(0xFF), 0xFFFD);
    }
}
