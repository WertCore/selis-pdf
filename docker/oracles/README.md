...
development. Otherwise the pinned container is dispatched through Docker. A
container without a recorded digest is refused: an unpinned oracle output is
not comparable.

## Building and recording digests

Docker is required (CI has it; the dev host that authored this tree does
not). To build and record a digest:
...
touches `docker/oracles/**` and smoke-renders `fixtures/smoke.pdf` through
each render oracle (plus a `qpdf --json` structural pass), so the recipes are
validated even before digests are recorded.
...
| Ghostscript 9.56.1 | **not verified locally** — gs is not installed and Docker is unavailable; the Dockerfile recipe is exercised in CI |
